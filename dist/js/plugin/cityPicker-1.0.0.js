...
(function ($, window) {
    ...
    Citypicker.prototype = {
        init: function () {
            ...
            if (config.renderMode) {
                //模拟>添加数据
                $selector.find('.listing').data('id', '1').eq(0).find('ul').html(effect.montage.apply(self, [config.dataJson, '1']));
            } else {
                //原生>添加数据
                $selector.find('.province').append(effect.montage.apply(self, [config.dataJson, '1']));
            }
